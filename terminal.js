...
function openViewer(title, markdown) {
  const clean = stripObsidian(markdown);
  viewerTitle.textContent = `// ${title}`;
  viewerBody.innerHTML = marked.parse(clean);
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape' && viewer.style.display !== 'none') closeViewer();
});

// ── Content renderers ─────────────────────────────────

function renderEntries(entries, titleCls = 'amber') {
  ...
  // print clickable entry titles — clicking opens the viewer
  const out = [['', '']];
  entries.forEach((entry, i) => {
    out.push([`  [${i + 1}] ${entry.title}`, titleCls]);
  });
  out.push(['', '']);
  out.push(['  click an entry to open it, or type its number', 'muted']);
  out.push(['', '']);

  // attach click handlers after a tick so the DOM elements exist
  setTimeout(() => {
    const spans = output.querySelectorAll('.line-' + titleCls);
    // only bind the most recently added batch
    const recent = Array.from(spans).slice(-entries.length);
    recent.forEach((el, i) => {
      el.style.cursor = 'pointer';
      el.addEventListener('click', () => openViewer(entries[i].title, entries[i].body));
      el.addEventListener('mouseenter', () => el.style.color = 'var(--green)');
...
function homeLines() {
  ...
  out.push(['', '']);
  out.push(['  click an entry or type its number', 'muted']);
  out.push(['', '']);
  setTimeout(() => {
    const spans = output.querySelectorAll('.line-bright');
    const recent = Array.from(spans).slice(-entries.length);
    recent.forEach((el, i) => {
      el.style.cursor = 'pointer';
      el.addEventListener('click', () => openViewer(entries[i].title, entries[i].body));
...
async function boot() {
  try {
    const res = await fetch('/PersonalSiteVK/content.json');
...
// ── Last viewed entries (for number selection) ────────
let lastEntries = [];

function showSubsection(sectionKey, subKey) {
  const meta = SECTION_META[sectionKey]?.subsections[subKey];
  if (!meta) return [['', ''], [`  unknown subcommand. try: ${sectionKey}`, 'error'], ['', '']];
...
function runCommand(raw) {
  ...
  // number input — open that entry from the last listing
  if (/^\d+$/.test(cmd)) {
  ...
  switch (cmd) {
    ...
    case 'projects':
      sub && SECTION_META.projects.subsections[sub]
        ? printLines(showSubsection('projects', sub))
        : sub
    ...
    case 'art':
      sub && SECTION_META.art.subsections[sub]
        ? printLines(showSubsection('art', sub))
        : sub
    ...
    case 'consumption':
      sub && SECTION_META.consumption.subsections[sub]
        ? printLines(showSubsection('consumption', sub))
        : sub
