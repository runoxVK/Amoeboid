...
function echoCmd(cmd) { line(`runox@requiem:~$ ${cmd}`, 'echo'); }
...
function renderEntries(entries, titleCls = 'amber') {
  ...
  for (const entry of entries) {
    out.push(['', '']);
    out.push([`  [ ${entry.title} ]`, titleCls]);
    if (entry.body) out.push(...renderMarkdown(entry.body));
    out.push(['  ─────────────────────────────────────────', 'muted']);
  }
  out.push(['', '']);
  return out;
...
async function boot() {
  ...
  await delay(350);
  await printLines([['', ''], ['initializing REQUIEM...', 'dim']], 55);
  await delay(450);
  await printLines([
